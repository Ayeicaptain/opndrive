...
export default function LoginPage() {
  ...
  const cognitoAuthorizePath =
    process.env.NEXT_PUBLIC_COGNITO_AUTHORIZE_PATH ?? '/oauth2/authorize';
  ...
  const baseLoginUrl = useMemo(() => {
    ...
    const url = new URL(cognitoAuthorizePath, normalizeCognitoDomain(cognitoDomain));
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-10 md:py-16">
        <div className="mx-auto w-full max-w-md rounded-xl border border-border bg-card p-6 md:p-8 shadow-sm">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            Authorize endpoint path: <code>{cognitoAuthorizePath}</code>
          </p>
        </div>
      </main>
    </div>
